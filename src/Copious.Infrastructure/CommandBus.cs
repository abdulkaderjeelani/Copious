...
namespace Copious.Infrastructure {
    public class CommandBus : ICommandBus {
        ...
        [DebuggerStepThrough]
        public Task SendAsAsync<T> (Envelope<T> command) where T : Command => Task.Factory.StartNew (() => Send (command));
        ...
        [DebuggerStepThrough]
        public void SendAsyncAsSync<T> (Envelope<T> command) where T : Command => Task.Run (async () => await SendAsync (command)).Wait ();
    }
}