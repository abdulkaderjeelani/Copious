...
namespace Copious.Application
{
    public class CommandHandler<TCommand, TAggregate, TState> : ICommandHandler<TCommand>, ICommandHandlerAsync<TCommand> where TCommand : Command where TAggregate : StatedAggregate<TAggregate, TState>, new() where TState : class,  IEntity<Guid>, new()
    {
        ...
        protected CommandHandler(ILoggerFactory loggerFactory, IExceptionHandler exceptionHandler, IRepository<TState> repository, ICommandGuard guard, IEventBus eventBus, IMapper mapper)
            : this(loggerFactory, exceptionHandler, repository, guard, eventBus)
        {
            _mapper = mapper;
        }
        ...
        protected TAggregate ExecuteCommand(TCommand command)
        {
            ...
            if (state == null) agg.Id = command.ComponentId; else agg.State = state;
