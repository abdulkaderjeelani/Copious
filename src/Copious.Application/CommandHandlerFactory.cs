...
namespace Copious.Application
{
    public sealed class CommandHandlerFactory : ICommandHandlerFactory
    {
        ...
        public IEnumerable<ICommandHandlerAsync<T>> GetAsyncHandlers<T>() where T : Command
           => MessageHandlerResolver.GetMessageHandlerType<T>(typeof(ICommandHandlerAsync<>))
        ...
        public IEnumerable<ICommandHandler<T>> GetHandlers<T>() where T : Command
            => MessageHandlerResolver.GetMessageHandlerType<T>(typeof(ICommandHandler<>))
              .Select(h => (ICommandHandler<T>)GetCommandHandlerInstance(h));

        private object GetCommandHandlerInstance(Type h)
