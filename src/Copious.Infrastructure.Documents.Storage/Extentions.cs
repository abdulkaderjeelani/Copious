...
namespace Copious.Infrastructure.Documents.Storage {
    public static class Extentions {
        ...
        public static StorageError ToStorageError (this int code) => ((StorageErrorCode) code).ToStorageError ();
        ...
        public static List<T2> SelectToListOrEmpty<T1, T2> (this IEnumerable<T1> e, Func<T1, T2> f) => e == null ? new List<T2> () : e.Select (f).ToList ();

        public static List<T1> WhereToListOrEmpty<T1> (this IEnumerable<T1> e, Func<T1, bool> f) => e == null ? new List<T1> () : e.Where (f).ToList ();
