...
namespace Copious.Persistance.EF {
    ...
    public class ReadonlyRepository<TState>:
        ...
        where TState : class, Identifiable<Guid>, new () {
            ...
            public virtual List<TState> GetAll () => DbSet.ToList ();

            public virtual async Task<List<TState>> GetAllAsync () => await DbSet.ToListAsync ();

            public virtual IAsyncEnumerable<TState> GetAllAsAsync () => DbSet.ToAsyncEnumerable ();

            public TState Get (Guid id) => DbSet.SingleOrDefault (i => i.Id == id);
        }
