...
namespace Copious.Persistance.EF {
    ...
    public class GeneralQueryHandler<TState> : EFBase<TState>,
        ...
        where TState : class, Identifiable<Guid>, new () {
            ...
            protected virtual IQueryable<TResult> Sort<TResult> (Query query, IQueryable<TResult> queryable) => (query.SortProperties != null) ?
                queryable.OrderBy (string.Join (",", query.SortProperties.Select (s => $"{s.Key} {s.Value}"))) : queryable;

            public List<TState> Fetch (GetAllQuery query) => ProtectedResult (DbSet.ToList ());

            public TState Fetch (FindByIdQuery query) => _readonlyRepository.Get (query.Id);

            public List<TState> Fetch (SearchQuery<TState> query) => ProtectedResult (DynamicLinqHelper.ApplyFeatures (query, DbSet.AsNoTracking ().Where (query.Predicate), _guard).ToList ());

            public async Task<List<TState>> FetchAsync (GetAllQuery query) => ProtectedResult (await _readonlyRepository.GetAllAsync ());

            public async Task<TState> FetchAsync (FindByIdQuery query) => await DbSet.SingleOrDefaultAsync (i => i.Id == query.Id);

            public async Task<List<TState>> FetchAsync (SearchQuery<TState> query) => ProtectedResult (await DynamicLinqHelper.ApplyFeatures (query, DbSet.AsNoTracking ().Where (query.Predicate), _guard).ToListAsync ());
