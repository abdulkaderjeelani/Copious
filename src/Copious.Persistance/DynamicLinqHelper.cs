...
namespace Copious.Persistance
{
    public static class DynamicLinqHelper
    {
        ...
        public static IQueryable<TResult> Sort<TResult>(Query query, IQueryable<TResult> queryable)
            => (query.SortProperties != null) ? queryable.OrderBy(string.Join(",", query.SortProperties.Select(s => $"{s.Key} {s.Value}"))) : queryable;
    }
}