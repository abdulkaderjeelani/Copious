...
namespace Copious.Infrastructure.Documents.Storage.Azure {
    public sealed class AzureStorageProvider : IStorageProvider {
        ...
        public async Task SaveBlobStreamAsync (string containerName, string blobName, Stream source, Documents.Storage.BlobProperties properties = null, bool closeStream = true) {
            var container = _blobClient.GetContainerReference (containerName);
            var props = properties ?? Documents.Storage.BlobProperties.Empty;
        ...
        public async Task UpdateBlobPropertiesAsync (string containerName, string blobName, Documents.Storage.BlobProperties properties) {
            var container = _blobClient.GetContainerReference (containerName);
            var blob = container.GetBlockBlobReference (blobName);
